...
# /home/raymond/local/src/git/SObA/CaoTestFile/SR1_vs_wt_BP.txt 
# /home/raymond/local/src/git/SObA/CaoTestFile/SR1_vs_wt_CC.txt 
# /home/raymond/local/src/git/SObA/CaoTestFile/SR1_vs_wt_MF.txt 
...
# NEW COMMENT OUT nodeWidth function until can scale off of P value
...
#   $annotation_count_solr_url = 'http://wobr2.caltech.edu:8080/solr/go/select?qt=standard&fl=topology_graph_json,regulates_closure&version=2.2&wt=json&indent=on&rows=1000&q=id:(%22GO:1903825%22%20OR%20%22GO:1905039%22%20OR%20%22GO:1901564%22)&fq=document_category:ontology_class';
...
#       last if ($count > $maxNodes);
      my $parent = shift @parentNodes;					# take a parent
# print qq(shift P $parent PN @parentNodes P\n);
      foreach my $child (sort keys %{ $edgesLca{$parent} }) {		# each child of parent
        $count++;
# print qq(NODE $child OF $parent C $count E\n);
        foreach my $type (keys %{ $nodes{$child} }) {
          $tempNodes{$child}{$type} = $nodes{$child}{$type}; }
        push @parentNodes, $child;					# child is a good node, add to parent list to check its children
# print qq(PUSH $child PN @parentNodes\n);
...
#   my $descriptionNodesWanted = 5;
#   my @mostNodes;
#   my (@parentNodes) = split/,/, $rootsChosen;
#   $descriptionTerms = @parentNodes;
  ...
  foreach my $source (sort keys %edgesLca) {
    foreach my $target (sort keys %{ $edgesLca{$source } }) {
#       if ( ($filterLongestFlag) && !($edgesFromLongest{$target}{$source}) ) { next; }		# separately processed above because needed before optional maxNodes trimming
...
# http://wobr2.caltech.edu/~azurebrd/cgi-bin/amigo.cgi?action=annotSummaryCytoscape&focusTermId=WBGene00000899
  ...
  if ($all_roots eq 'all_roots') { 
      $fakeRootFlag = 1; $filterLongestFlag = 1; $filterForLcaFlag = 1; $maxNodes = 0;
...
//     var start = '0000000';
//     for( var i = 0; i < parentToChild[start].length; i++ ){
//       var newObj = parentToChild[start][i];
...
<div style="width: 1705px;">
  ...
  <div id="controldiv" style="z-index: 9999; border: 1px solid #aaa; position: relative; float: left; width: 200px; display: none;">
    ...
    <form method="get" action="soba_enriched.cgi">
...
sub makeLink {
  ...
  my $url = "soba_enriched.cgi?action=Tree&focusTermId=$focusTermId";
