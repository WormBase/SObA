...
my $outfile = "output.txt.$date.lifestage";
open(my $out, '>', $outfile) or die "Cannot open $outfile : $!";

my $errfile = "errorfile.txt.$date.lifestage";
open(ERR, ">$errfile") or die "Cannot create $errfile : $!";

my $url = 'http://wobr2.caltech.edu:8080/solr/lifestage/select?qt=standard&indent=on&wt=json&version=2.2&fl=id&start=0&rows=100000&q=document_category:bioentity&fq=taxon:%22NCBITaxon:6239%22';
...
#   next unless ($id =~ m/Q8N6N2/);
  my $all_url = "http://wobr2.caltech.edu/~azurebrd/cgi-bin/soba_biggo.cgi?action=annotSummaryJson&focusTermId=$id&datatype=lifestage&radio_etgo=&rootsChosen=WBls:0000075&showControlsFlag=1&fakeRootFlag=0&filterForLcaFlag=0&filterLongestFlag=0&maxNodes=0&maxDepth=0";
  my $failCount = 0;
  my ($nodesAll, $directAll, $edgesAll) = (0, 0, 0);
  while ($nodesAll < 1 && $failCount < 10) {
    ($nodesAll, $directAll, $edgesAll) = &getNodesEdges( $all_url ); }
  $failCount = 0;
  my $f1_url = "http://wobr2.caltech.edu/~azurebrd/cgi-bin/soba_biggo.cgi?action=annotSummaryJson&focusTermId=$id&datatype=lifestage&radio_etgo=&rootsChosen=WBls:0000075&showControlsFlag=1&fakeRootFlag=0&filterForLcaFlag=1&filterLongestFlag=0&maxNodes=0&maxDepth=0";
  my ($nodesF1, $directF1, $edgesF1) = (0, 0, 0);
  while ($nodesF1 < 1 && $failCount < 10) {
    ($nodesF1, $directF1, $edgesF1) = &getNodesEdges( $f1_url ); }
  $failCount = 0;
  my $f2_url = "http://wobr2.caltech.edu/~azurebrd/cgi-bin/soba_biggo.cgi?action=annotSummaryJson&focusTermId=$id&datatype=lifestage&radio_etgo=&rootsChosen=WBls:0000075&showControlsFlag=1&fakeRootFlag=0&filterForLcaFlag=1&filterLongestFlag=1&maxNodes=0&maxDepth=0";
